...
public class CeButton extends JButton {
	...
	public CeButton(String text) {
		...
		this.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent e) {
				self.setIcon(img_icon);
			}

			@Override
			public void mousePressed(MouseEvent e) {
				self.setIcon(img_on_icon);
			}
			...
			public void mouseEntered(MouseEvent e) {
				self.setIcon(img_mouseOver_icon);
			}
	...
	public void imageReflesh() {
		ArrayList<File> list = FileOperator.listFiles(new File("./tmp"));
		for (File file : list) {
			if (file.getName().equals(String.valueOf(text.hashCode()))) {
				CVImage img = new CVImage("./tmp/" + text.hashCode());
				if (img.getHeight() == this.getHeight() && img.getWidth() == this.getWidth()) {
					String imgName = "./tmp/" + text.hashCode();
					String imgOnName = "./tmp/" + (text + "on").hashCode();
					String imgMouseOver = "./tmp/" + (text + "mouseOver").hashCode();
